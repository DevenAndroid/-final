...
import '../model/model_order_details.dart';
...
class StoreOrderController extends GetxController
  ...
  Future<void> _getStoreOrders(timeStatus,{String? filterBy}) async {
    ...
    await _repository.getStoreOrders(
        page, repositoriesPerPage, timeStatus , filterBy: filterBy).then((result) {
  ...
  Rx<ModelOrderDetails> model = ModelOrderDetails().obs;

  Future<StoreOrderDetail> getOrderDetailsById(int? id) async {
   await orderDetails(id).then((value) {
      model.value = value;
    });
    return await _repository.getOrderDetailsById(id);
  }
